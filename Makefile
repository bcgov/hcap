...
build-mongo-db-backup:
	@oc -n $(TOOLS_NAMESPACE) process -f openshift/universal.bc.yml \
		-p NAME=$(APP_NAME)-backup-mongo \
		-p TAG="latest" \
		...
		-p SOURCE_CONTEXT_DIR="docker" | oc -n $(TOOLS_NAMESPACE) apply -f -
	@oc -n $(TOOLS_NAMESPACE) start-build bc/$(APP_NAME)-backup-mongo --wait

deploy-mongo-db-backup:
	@oc -n $(TARGET_NAMESPACE) process -f openshift/backup-mongo.dc.yml \
		-p NAME=$(APP_NAME)-backup-mongo \
		-p IMAGE_STREAM_TAG=$(APP_NAME)-backup-mongo:latest \
		-p BUILD_NAMESPACE=$(TOOLS_NAMESPACE) \
		-p DB_NAME=$(APP_NAME)  | oc -n $(TARGET_NAMESPACE) apply -f -
