...
exports.up = async () => {
  await dbClient.db.withTransaction(async (tx) => {
    await tx.query(
      `UPDATE ${collections.EMPLOYER_SITES} SET body = body - 'earlyAdopterAllocation' || jsonb_build_object('phaseOneAllocation', body->'earlyAdopterAllocation')`
    );

    await tx[collections.EMPLOYER_SITES].updateDoc({ postalCode: 'A1A 1A1' }, { postalCode: '' });
